...
class LatentEditor(object):
    def __init__(self, stylegan_generator, encoder, is_cars=False):
        self.generator = stylegan_generator
        self.encoder = encoder
        self.is_cars = is_cars  # Since the cars StyleGAN output is 384x512, there is a need to crop the 512x512 output.

    def apply_ganspace(self, latent, image, ganspace_pca, edit_directions):
        edit_latents = ganspace.edit(latent, ganspace_pca, edit_directions)
        return self._latents_to_image(edit_latents, image)

    def apply_interfacegan(self, latent, image, direction, factor=1, factor_range=None):
        edit_latents = []
    ...
    def apply_sefa(self, latent, image, indices=[2, 3, 4, 5], **kwargs):
    ...
    def _latents_to_image(self, latents, image):
        with torch.no_grad():
            _, contents = self.encoder(image)
            images, _ = self.generator([latents], contents, randomize_noise=False, input_is_latent=True)
            if self.is_cars:
